    ...
    def get(self):
        ...
        try:
            ...
            response = (
                "<?xml version=\"1.0\" encoding=\"UTF-8\" ?>"
                "<Response>"
                "        <Message>The system has accepted your message and created an incident. On call staff are being contacted now. You will be notified when the incident is acknowledge and when it is marked resolved. Sending additional messages will open additional incidents, please do this only after this incident is marked resolved and if you believe it is not yet resolved. If you need to provide additional details please do so in the ticket system: " + os.environ['TICKET_SYSTEM_URL'] + "</Message>"
                "</Response>")
            logging.debug("response = \"" + response + "\"")
            self.response.out.write(response)
